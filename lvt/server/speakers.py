...
class Speaker():
    def __init__( self, speakerId: str, configParser: ConfigParser ):
        self.id = speakerId
        signature = configParser.getValue( '','Signature','' ).strip()
        if signature=='' : raise Exception('Speaker signature is empty!')
        self.signature = json.loads(signature)

        self.name = configParser.getValue( '','Name',self.id )
        self.isAdmin = configParser.getIntValue( '', 'isAdmin', 0 ) == 1
        self.email = configParser.getValue( '','email', '' )
